...
const FullPost = ({ userId}) =>
        ...
        useEffect(() => {
            axios.get(`/posts/productId=${postId}`,{withCredentials : true})
            .then(res => {
        ...
        return(
            <div>
                <Header publication={true} Title={Title} />
                {loading &&(
                    <div className="FullPostContainer">
                        <Skeleton loading={true} active avatar paragraph={{ rows: 0 }}>
                        </Skeleton>
                        <Skeleton loading={true} active title={false} height="180px" paragraph={{ rows: 8 }}>
                        </Skeleton>
                    </div>
                )}
                {!loading && (
                    <div className="FullPostContainer">
                        ...
                        <FooterContainer post={post} userId={userId}/>
                    </div>
                )}
                <div className="ReportContainer">
                        ...
                        <span className="span">Signaler l'annonce</span>
                </div>
            </div>
